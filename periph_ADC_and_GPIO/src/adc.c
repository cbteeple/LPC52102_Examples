...
#define TICKRATE_HZ (2000)	/* 100 ticks per second */
...
void flipBit(void){
	int i;
	for (i = 0; i < sizeof(GPIOPins); i++) {
		Chip_GPIO_SetPinToggle(LPC_GPIO, 0, GPIOPins[i]);
	}
		...
		//	sequenceComplete = false;
			...
			/* Get raw sample data for channels 0-11 */
				...
				//if ((rawSample & (ADC_DR_OVERRUN | ADC_SEQ_GDAT_DATAVALID)) != 0) {
					//DEBUGOUT("Sample value = %d \t", ADC_DR_RESULT(rawSample));
